...
function SubCategoryButton({category, onSubCategoryChange, selectedSubCategory}) {

  const [selectedCategory, setSelectedCategory] = useState(selectedSubCategory || "TOTAL");
  const { t } = useTranslation();

  const categories = getCategories(category);

  useEffect(() => {
    if (selectedSubCategory !== undefined && selectedSubCategory !== selectedCategory) {
